...
class Bot{
    public:
        ...
        std::string middle_game_move_threading(int depth, Board& board, char colour);
    ...
    private:
        ...
        float minimax2(int depth, float alpha, float beta, bool maximizing_player, Board board);
        float quiescence(Board& board, float alpha, float beta, int depth);
        float evaluate2(const Board& board);
