...
for i in range(10):
    print(str(bot) + "\n")
    print(bot.evaluateThePosition(bot.getBoard()))
    if turn == 0:
        start = time()
        bot.getBoard().push_san(bot.getBestMove(5, bot.getBoard(), "w"))
        print("White time taken: ", time() - start)
        turn = 1
    elif turn == 1:
        start = time()
        bot.getBoard().push_san(bot.getBestMove(5, bot.getBoard(), "b"))
