...
def play_game(engine1_path, engine2_path, p1, p2, fen="rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"):
    ...
    try:
        ...
        while not board.is_game_over():
            # Engine 1's move
            result = engine1.play(board, chess.engine.Limit(time=5.0)) #? chess.engine.Limit(time=2.0) in seconds
            ...
            # Engine 2's move
            result = engine2.play(board, chess.engine.Limit(time=5.0)) #? chess.engine.Limit(time=2.0) in seconds
            board.push(result.move)
...
engine1_path = "engines\\engine-notaper.exe"
engine2_path = "engines\\engine-taper.exe"

engine1_name = "engine-notaper"
engine2_name = "engine-taper"
...
# play_game(engine1_path, engine2_path, engine1_name, engine2_name)
