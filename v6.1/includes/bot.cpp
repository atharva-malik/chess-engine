...
Bot::Bot() {
    ...
    this->game_stage = 'o';
    this->evaluation = Bot::evaluate(this->board);
}
...
std::string Bot::get_best_move(int depth, Board& board, char colour) {
    if (this->game_stage == 'o') return Bot::get_opening_move(board.getFen());
        ...
        return this->middle_game_move(depth, board, colour); // TODO: Implement endgame move selection
    }
}
...
float Bot::minimax(int depth, float alpha, float beta, bool maximizing_player, Board& board){
    ...
    else if (depth == 0) return this->better_evaluate(board);
...
void Bot::move(std::string uci_move, Board& board){
    Move move = uci::uciToMove(this->board, uci_move);
    std::cout << "move: " << board.at(Square(Square::underlying::SQ_E4)) << std::endl;
    if (board.isCapture(move)){
        this->evaluation = Bot::evaluate(board);
        return;
    }
    Square sq_from = move.from();
    Square sq_to = move.to();
    switch (board.at(sq_from)){
        case 12:
...
float Bot::evaluate(Board& board){
    ...
    for (int rank = 0; rank < 8; ++rank) {
        for (int file = 0; file < 8; ++file) {
            ...
            score += this->piece_tables.w_pawn[rank][file] * w_pawns.check(square_no);
            score -= this->piece_tables.b_pawn[rank][file] * b_pawns.check(square_no);
            score += this->piece_tables.w_rook[rank][file] * w_rooks.check(square_no);
            score -= this->piece_tables.b_rook[rank][file] * b_rooks.check(square_no);
            score += this->piece_tables.w_knight[rank][file] * w_knights.check(square_no);
            score -= this->piece_tables.b_knight[rank][file] * b_knights.check(square_no);
            score += this->piece_tables.w_bishop[rank][file] * w_bishops.check(square_no);
            score -= this->piece_tables.b_bishop[rank][file] * b_bishops.check(square_no);
            score += this->piece_tables.w_queen[rank][file] * w_queens.check(square_no);
            score -= this->piece_tables.b_queen[rank][file] * b_queens.check(square_no);
            if (this->game_stage != 'e') score += this->piece_tables.w_king_mid[rank][file] * w_king.check(square_no);
            else score += this->piece_tables.w_king_end[rank][file] * w_king.check(square_no);
            if (this->game_stage != 'e') score -= this->piece_tables.b_king_mid[rank][file] * b_king.check(square_no);
            else score -= this->piece_tables.b_king_end[rank][file] * b_king.check(square_no);
        }
    }
...
float Bot::better_evaluate(Board& board){
    int score = 0;
